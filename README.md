<!-- Visitor badge -->
<img align="right" src="https://visitor-badge.laobi.icu/badge?page_id=AbdelrahmanSalhab.AbdelrahmanSalhab">

<h1 align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.herokuapp.com/?lines=Hello,+there!+👋;I'm+Abdelrahman;Nice+to+meet+you!&center=true&size=30">
  </a>
</h1>

<p align="center">
  Hi — I'm Abdelrahman (GitHub: <a href="https://github.com/AbdelrahmanSalhab">@AbdelrahmanSalhab</a>). I'm a software developer based in Jerusalem.
  <br><br>
  📫 How to reach me: <code>abdrhmn.salhab@gmail.com</code>
  <br>
  📦 Public repositories: 5
</p>

<hr>

## Connect with me

- GitHub: https://github.com/AbdelrahmanSalhab
- LinkedIn: https://www.linkedin.com/in/abdelrahman-salhab/
<hr>

## Languages & Tools
These are suggested items — please update to match your actual stack.

- Languages: Java · Python · C
- Web / Backend: PHP
- Databases: MySQL
- Tools: Git · Docker · VS Code
- Other: Algorithms · Data Structures · Problem solving
<hr>

## Stats

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=AbdelrahmanSalhab&show_icons=true&theme=react" alt="AbdelrahmanSalhab's GitHub stats" width="48%"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=AbdelrahmanSalhab&layout=compact&theme=react" alt="Top languages" width="48%"/>
</div>

<hr>

## Featured repositories
Please replace these example entries with the real repo links and one-line descriptions (I couldn't retrieve repo names in this context).

- [repo-1-name](https://github.com/AbdelrahmanSalhab/Crop-Recommendation-System) — A simple Java app that uses a Decision Tree (trained on a Kaggle dataset of 2,201 records) to recommend crops. Includes a basic GUI for entering soil and weather features. Reported accuracy: 98.91%.
- [repo-2-name](https://github.com/AbdelrahmanSalhab/Travelers-Guide) — A JavaFX-Based application that relies on Dijkstra's Algorithm to find the optimal path within your chosen criteria.

<hr>
